...
**アカウントテーマ: 「占い動物園・主は猫族の家」 — 3種の猫がタロットで占い、他動物がゲスト鑑定士として日替わり登場。最後に鳴き声で締める。**

- **主軸 (70%)**: 3種の猫 × タロット (テーマ専属化)
  - 🖤 **黒猫**: 恋愛・本音・運命系 (主役)
  - 🧡 **三毛猫**: 金運・仕事運・幸運系
  - 💜 **シャム猫**: 決断・キャリア・選択系
- **ゲスト枠 (30%)**: うさぎ🐇 (パワーストーン)・フクロウ🦉 (星読み)・狐🦊 (鍵)・ハリネズミ🦔 (ハーブ) など
- 全動画 **15秒・4カット・コメント誘導型** で統一 (= `prompts/script_user_3choice_short.md`)
- 各キャラの鳴き声 (にゃおー・にゃ!・みゃおうっ・きゅう・ホー…) がアカウントのブランドサウンド
- 結果はピン留めコメントで開示 → コメント率と再視聴率を最大化

詳細は `README.md` `docs/strategy.md` `config/cast.yaml` `config/tools.yaml` `prompts/image_cat_characters.md` 参照。
